//TODO special .h file with error messages & error values!
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
...
void openPipes(int *pipe_in, int *pipe_out, const int argc, const char **argv);
...
int main(const int argc, const char *argv[]){
  ...
  enum{KEYBOARD_THRD,READ_PIPE_THRD,MAIN_THRD, NUM_THRDS};
  const char *thrd_names[]={"Keyboard","ReadPipe","Main"};
  void* (*thrd_functions[])(void*)={keyboard_thread,read_pipe_thread,mainThread};
...
void* read_pipe_thread(void *d){
  ...
  unsigned char c;
  while(io_getc_timeout(pipe_in, IO_READ_TIMEOUT_MS, &c) > 0); //clear pipeline garbage
