...
// - function  ----------------------------------------------------------------
bool get_message_size(uint8_t msg_type, size_t *len){
   bool ret = true;
   switch(msg_type) {
      case MSG_OK:
      case MSG_ERROR:
...
// - function  ----------------------------------------------------------------
bool fill_message_buf(const message *msg, uint8_t *buf, size_t size, size_t *len){
   ...
   switch(msg->type) {
      ...
      case MSG_GET_VERSION:
         *len = 1;
         break;
      case MSG_STARTUP:
         for (size_t i = 0; i < STARTUP_MSG_LEN; ++i)
...
// - function  ----------------------------------------------------------------
bool parse_message_buf(const uint8_t *buf, size_t size, message *msg){
   uint8_t cksum = 0;
